...
Its training set is generated by `scripts/augment_slot_training_data.py` from
deterministic paraphrase templates. The generated JSONL reinforces complete
empty and multi-slot JSON objects while preserving the original instruction
file. Normalized text overlap with `data/evaluation/slot_eval_cases.jsonl` is a
hard validation error, so the hold-out set remains separate from augmentation.
After retraining, compare strict parse success, weak repairs, trusted slots,
meaningful LLM slot contributions and mean latency across the matrix rows.
